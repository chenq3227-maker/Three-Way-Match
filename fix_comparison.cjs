...
const regex = /\{\/\* Visual Mismatch Alert Header \*\/\}\n\s*\{selectedLine\.overallStatus !== "Awaiting Department Approval" && \([\s\S]*?\)\}\n\s*<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">/;

code = code.replace(regex, `{\/\* Visual Mismatch Alert Header \*\/}
                    ...
                    <div className="space-y-3">
                      {selectedLine.exceptions && selectedLine.exceptions.length > 0 ? (
                        ...
                        <div className={\`bg-white/80 p-3 rounded-lg border space-y-1 \${
                          ...
                          <div className="text-gray-700">Please verify details manually.</div>
                        </div>
                      )}
                    </div>
                  </div>
